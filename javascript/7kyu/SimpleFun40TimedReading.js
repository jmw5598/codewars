/*
  Task:
    ...
    a dvance reading skills. A young elementary student has just finished his
    very first timed reading exercise. Unfortunately he's not a very good reader
    yet, so whenever he encountered a word longer than maxLength, he simply
...
function timedReading(maxLength, text) {
  return text.replace(/[^a-z\s]/gi, "")
    .split(/\s/g)
    .filter(e => e.length && e.length <= maxLength)
    .length;
}
