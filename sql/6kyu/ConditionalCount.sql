/*
  Given a payment table, which is a part of DVD Rental Sample Database, with
  ...
  produce a result set for the report that shows a side-by-side comparison of
  the number and total amounts of payments made in Mike's and Jon's stores
  broken down by months.

  The resulting data set should be ordered by month using natural order
  (Jan, Feb, Mar, etc.).

  Note: You don't need to worry about the year component. Months are never
  repeated because the sample data set contains payment information only for
  one year.
...
SELECT EXTRACT('month' FROM p.payment_date) AS "month",
  ...
  COUNT(CASE WHEN p.staff_id = 1 THEN 1 END) AS mike_count,
  SUM(CASE WHEN p.staff_id = 1 THEN p.amount END) AS mike_amount,
  COUNT(CASE WHEN p.staff_id = 2 THEN 1 END) AS jon_count,
  SUM(CASE WHEN p.staff_id = 2 THEN p.amount END) AS jon_amount
FROM payment AS p
GROUP BY "month"
ORDER BY "month"
